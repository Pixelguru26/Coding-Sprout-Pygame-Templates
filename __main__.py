...
while True: # All game end conditions currently break from the loop, so while true is safe.
  for event in pygame.event.get():
    if event.type == QUIT:
      ...
      core.keydown(event.key, event.mod, event.unicode, event.scancode)
    elif event.type == KEYUP:
      if script_ku:
        script.keyup(event.key, event.mod, event.unicode, event.scancode)
      core.keyup(event.key, event.mod, event.unicode, event.scancode)
    elif event.type == MOUSEBUTTONDOWN:
      if script_md:
        script.mousedown(event.button, event.pos[0], event.pos[1])
      core.mousedown(event.button, event.pos[0], event.pos[1])
    elif event.type == MOUSEBUTTONUP:
      if script_mu:
        script.mouseup(event.button, event.pos[0], event.pos[1])
      core.mouseup(event.button, event.pos[0], event.pos[1])
